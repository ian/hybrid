 ...
 */
export const launchMiniappTool = (opts: { url: string }) =>
	createTool({
		id: "launchMiniapp",
		...
		execute: async ({ input, runtime }) => {
			...
			logger.debug(
				`🚀 [Tool:launchMiniapp] Starting execution with URL: "${opts.url}"`
			)
			...
			try {
				...
				logger.debug(`🚀 [launchMiniapp] Sending miniapp URL: "${opts.url}"`)

				const sendStartTime = performance.now()
				const messageId = await conversation.send(opts.url)
 ...
 */
export const miniAppTools = (opts: { url: string }) => ({
	launchMiniapp: launchMiniappTool(opts)
})
